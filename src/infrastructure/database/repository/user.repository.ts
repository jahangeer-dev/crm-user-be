...
export class UserRepository {
    ...
    public async getByEmail(email: string): Promise<GetUserResponseDTO | null> {
        let user = await this.redisclient.get(email) as (IUser & { _id: string }) | null;
        ...
        if (!user) return null;
        this.redisclient.set(user.email, JSON.stringify(user), 60 * 60 * 24)
        return {
            id: user._id as string,
    ...
    public async getByUserName(userName: string): Promise<GetUserResponseDTO | null> {
